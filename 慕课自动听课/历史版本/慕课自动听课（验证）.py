...
def init():
    ...
    while i<1:
        ...
        referer="http://www.neumooc.com/course/play/init?courseId=C67744D352EB42B199D5905E7167AB97"
...
def linsten_init(session,courseId):
    ...
    i=1
    while i<1:
        url="http://www.neumooc.com/course/play/init?courseId=C67744D352EB42B199D5905E7167AB97&outlineId="+str(outlineId_grup[len(outlineId_grup)-1])
        try:
            response=session.get(url)
    ...
    while e < len(outlineId_grup):
        ...
        time.sleep(0.3)
    _thread.start_new_thread(threadcheck,())
...
def linsten(session,outlineId_grup,tittle,e,courseId,statut):
    ...
    url="http://www.neumooc.com/course/play/init?courseId=C67744D352EB42B199D5905E7167AB97&outlineId="+str(outlineId)
    i=0
    while i<1:
        try:
            response=session.get(url)
    ...
    session.post(url,data)
    xcmax+=1
    if e==len(outlineId_grup)-1 and statunum==len(statut)-1 and xcmax==sdmax:
        last=True
